			      ...
			      enum notify_payload_hash_algorithms rsa_hash_algo)
{
	...
	case IKEv2_AUTH_HASH_SHA1:
		...
		break;
	case IKEv2_AUTH_HASH_SHA2_256:
		hash_digest_size = SHA2_256_DIGEST_SIZE + 1;
		break;
	case IKEv2_AUTH_HASH_SHA2_384:
	...
	unsigned char *signed_octets = alloc_bytes(hash_digest_size + 1, "signed octets size");
...

	{
		...
		u_char sig_val[RSA_MAX_OCTETS] = {0};
		int shr;

		shr = sign_hash(k, signed_octets, signed_len, sig_val, sz, TRUE, rsa_hash_algo);
		if (shr == 0)
			return FALSE;
		passert(shr == (int)sz);
				...
				enum notify_payload_hash_algorithms rsa_hash_algo)
{
	...
	unsigned char *calc_hash = alloc_bytes(hash_len + 1, "hash size");
