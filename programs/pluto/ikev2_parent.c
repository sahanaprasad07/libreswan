...
static stf_status ikev2_send_asn_hash_blob(enum notify_payload_hash_algorithms hash_algo, pb_stream *a_pbs);

static stf_status ikev2_check_asn_hash_blob(enum notify_payload_hash_algorithms hash_algo, pb_stream *a_pbs);

static stf_status asn_hash_out(const struct asn_hash_blob *asn_hash_blob, pb_stream *a_pbs);

static bool asn_hash_in(const struct asn_hash_blob *asn_hash_blob, pb_stream *a_pbs,
		   uint8_t size, uint8_t asn_blob_len);
...
static stf_status ikev2_send_asn_hash_blob(enum notify_payload_hash_algorithms hash_algo, pb_stream *a_pbs)
{
	...
	switch ( hash_algo )	{
	case IKEv2_AUTH_HASH_SHA2_256:
		sendstat = asn_hash_out(&asn_rsa_pss_sha2_256,a_pbs);
		if (sendstat != STF_OK) {
	...
	case IKEv2_AUTH_HASH_SHA2_384:
		...
		break;
	case IKEv2_AUTH_HASH_SHA2_512:
		sendstat = asn_hash_out(&asn_rsa_pss_sha2_512,a_pbs);
		if (sendstat != STF_OK) {
			return STF_INTERNAL_ERROR;
		}
		break;
...
static stf_status ikev2_check_asn_hash_blob(enum notify_payload_hash_algorithms hash_algo, pb_stream *a_pbs)
{
	switch ( hash_algo )	{
	case IKEv2_AUTH_HASH_SHA2_256:
		if (!(asn_hash_in(&asn_rsa_pss_sha2_256,a_pbs, ASN1_LEN_ALGO_IDENTIFIER, ASN1_SHA2_RSA_PSS_SIZE))) {
			return STF_FAIL;
		}
		break;
	case IKEv2_AUTH_HASH_SHA2_384:
		...
		break;
	case IKEv2_AUTH_HASH_SHA2_512:
		if (!(asn_hash_in(&asn_rsa_pss_sha2_512, a_pbs, ASN1_LEN_ALGO_IDENTIFIER, ASN1_SHA2_RSA_PSS_SIZE))) {
			return STF_FAIL;
		}
		break;
	default:
...
static stf_status asn_hash_out(const struct asn_hash_blob *asn_hash_blob, pb_stream *a_pbs)
{
	if (!out_raw(asn_hash_blob->size_blob, asn_hash_blob->size, a_pbs,
		"Length of the ASN.1 Algorithm Identifier")) {
		loglog(RC_LOG_SERIOUS, "DigSig: failed to emit ASN.1 Algorithm Identifier length");
		return STF_INTERNAL_ERROR;
	}
	if (!out_raw(asn_hash_blob->asn_blob, asn_hash_blob->asn_blob_len, a_pbs,
		   ...
		   uint8_t size, uint8_t asn_blob_len)
{
	...
	uint8_t check_blob[asn_blob_len];


	if (!in_raw(check_size, size, a_pbs,
		"Algorithm Identifier length"))
		return FALSE;
	if (!memeq(check_size, asn_hash_blob->size_blob, asn_hash_blob->size))	{
		loglog(RC_LOG_SERIOUS, " Received incorrect size of ASN.1 Algorithm Identifier");
		return FALSE;
	}

	if (!in_raw(check_blob, asn_blob_len, a_pbs,
		"Algorithm Identifier value"))
		return FALSE;
	if (!memeq(check_blob, asn_hash_blob->asn_blob, asn_hash_blob->asn_blob_len))	{
	...
	const enum keyword_authby that_authby)
{
	...
	case IKEv2_AUTH_DIGSIG:
	{
		...
		stf_status checkstat = ikev2_check_asn_hash_blob(hash_algo,pbs);

		if ((checkstat != STF_OK) && (st->st_hash_negotiated & NEGOTIATE_AUTH_HASH_SHA2_384) &&
					(!hash_check)) {
			hash_algo = IKEv2_AUTH_HASH_SHA2_384;
			checkstat = ikev2_check_asn_hash_blob(hash_algo,pbs);
		}
				  ...
				  chunk_t *null_auth)
{
	...
	case IKEv2_AUTH_DIGSIG:
	{
			...
			stf_status sendstat =ikev2_send_asn_hash_blob(hash_algo, &a_pbs);
			if (sendstat != STF_OK ) {
				return STF_FAIL;
			}
