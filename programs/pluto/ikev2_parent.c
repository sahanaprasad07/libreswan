				  ...
				  pb_stream *outpbs)
{
	...
	switch (a.isaa_type) {
	case IKEv2_AUTH_RSA:
		if (!ikev2_calculate_rsa_hash(pst, role, idhash_out, &a_pbs, IKEv2_AUTH_HASH_SHA1)) {
	...
	case IKEv2_AUTH_DIGSIG:
		if (pst->st_hash_negotiated & NEGOTIATE_AUTH_HASH_SHA2_256) {
			...
			if (!ikev2_calculate_rsa_hash(pst, role, idhash_out, &a_pbs, IKEv2_AUTH_HASH_SHA2_256)) {
